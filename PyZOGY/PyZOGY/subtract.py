...
def calculate_difference_image(science, reference, gain_ratio=np.inf, gain_mask=None, sigma_cut=5., use_pixels=False,
    ...
    if gain_ratio == np.inf:
        ...
        science.zero_point = util.solve_iteratively(science, reference, sigma_cut=sigma_cut, use_pixels=use_pixels,
                                                    ...
                                                    pixstack_limit=pixstack_limit,sep_deblend_nthresh=sep_deblend_nthresh,fit_noise_mode=fit_noise_mode)
    else:
...
def source_noise(image, kernel):
    ...
    """
    logging.info('Calculating source noise for noise_corrected image.')
    if image.variance is None:
        logging.warning('image.variance not found. Use image.raw_image+image.read_noise instead.')
        image.variance = np.copy(image.raw_image) + image.read_noise
    logging.debug('image.variance shape :',image.variance.shape)
    # logging.debug('try running fft2 on image.variance:',np.fft.fft2(image.variance))
...
def run_subtraction(science_image, reference_image, science_psf, reference_psf, output=None,
                    ...
                    gain_ratio=np.inf, gain_mask=None, use_pixels=False, sigma_cut=5., show=False, percent=95,
                    corrected=False, use_mask_for_gain=True, max_iterations=5, size_cut=False, pixstack_limit=None,sep_deblend_nthresh=32,fit_noise_mode='iqr',
                    logname=None):
    """
    ...
    difference = calculate_difference_image(science, reference, gain_ratio, gain_mask, sigma_cut, use_pixels, show,
                                            percent, use_mask_for_gain, max_iterations, size_cut, pixstack_limit,sep_deblend_nthresh,fit_noise_mode)
