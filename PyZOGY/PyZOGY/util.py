...
def fit_noise(data, n_stamps=1, mode='iqr', fname=''):
    ...
    if mode == 'sep':
        background = sep.Background(np.ascontiguousarray((data.data)))
...
def interpolate_bad_pixels(image, median_size=6, fname=''):
    ...
    # interpolated_image = interpolate_replace_nans(interpolated_image, Gaussian2DKernel(median_size))
...
def join_images(science_raw, science_mask, reference_raw, reference_mask, sigma_cut=5., use_pixels=False, show=False,
                percent=99, size_cut=True, pixstack_limit=None,sep_deblend_nthresh = 32, fit_noise_mode='iqr'):
    ...
    science_std, _ = fit_noise(science,mode=fit_noise_mode)
    reference_std, _ = fit_noise(reference,mode=fit_noise_mode)
    if use_pixels:
        ...
        science_sources = sep.extract(np.ascontiguousarray(science.data), thresh=sigma_cut, err=science_std, mask=np.ascontiguousarray(science.mask),deblend_nthresh = sep_deblend_nthresh)
        reference_sources = sep.extract(np.ascontiguousarray(reference.data), thresh=sigma_cut, err=reference_std, mask=np.ascontiguousarray(reference.mask),deblend_nthresh = sep_deblend_nthresh)
...
def solve_iteratively(science, reference, mask_tolerance=10e-5, gain_tolerance=10e-6, max_iterations=5,
                      ...
                      pixstack_limit=None,sep_deblend_nthresh=32, fit_noise_mode='iqr'):
    ...
    while abs(gain - gain0) > gain_tolerance:
        ...
        x, y = join_images(science_convolved_image, science_mask_convolved, reference_convolved_image, 
                           reference_mask_convolved, sigma_cut, use_pixels, show, percent, size_cut, pixstack_limit,sep_deblend_nthresh, fit_noise_mode)
