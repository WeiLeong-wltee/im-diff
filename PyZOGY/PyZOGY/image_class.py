...
class ImageClass(np.ndarray):
    ...
    def __new__(cls, image_filename, psf_filename, mask_filename=None, n_stamps=1,
                saturation=np.inf, variance=None, read_noise=0, registration_noise=(0, 0),background_noise_mode='iqr',interpolate_bad_pixels=True):
        ...
        background_std, background_counts = util.fit_noise(masked_image, n_stamps=n_stamps, fname=image_filename,mode=background_noise_mode)
        if interpolate_bad_pixels:
            ...
            image_data = masked_image.astype(float).filled(np.nan)-background_counts
            image_data = np.where(np.isnan(image_data),0,image_data)
